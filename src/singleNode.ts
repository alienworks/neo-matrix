import fs from "fs"
import { Readable } from "stream"
import { MatrixRPC } from "./matricRpc"

// Constants
// a number constant use for retrying request, 3 is sufficient enough add more if you are not sure
const initialRetryTime = 5


// Interfaces
interface SingleNodeResponse {
    status: boolean | string
    response?: any
}

interface SingleNodeConfig {
    transformer?: () => any
}

// Helper functions
function singleNodeResponse(status: boolean | string, response?: any): SingleNodeResponse {
    return { status, response }
}

function pad(n: number) {
    return n.toString().padStart(2, '0')
}

function currentTime() {
    const now = new Date()

    return now.getFullYear() +
        '-' + pad( now.getMonth() + 1 ) +
        '-' + pad( now.getDate() ) +
        'T' + pad( now.getHours() ) +
        '_' + pad( now.getMinutes() ) +
        '_' + pad( now.getSeconds() ) +
        '_' + (now.getMilliseconds() / 1000).toFixed(3).slice(2, 5) +
        'Z'
}

function createLogFile() {
    return `${currentTime()}-error.log`
}


// Main Class TODO: add transformer
export class SingleNode {
    private client: MatrixRPC

    public logFileName: string

    // Default common parameter use for API
    public address = "AJBENSwajTzQtwyJFkiJSv7MAaaMc7DsRz"
    public assetID = "c56f33fc6ecfcd0c225c4ab356fee59390af8560be0e930faebe74a6daff7c9b"
    public hash = "773dd2dae4a9c9275290f89b56e67d7363ea4826dfd4fc13cc01cf73a44b0d0e"
    public txid = "92b1ecc0e8ca8d6b03db7fe6297ed38aa5578b3e6316c0526b414b453c89e20d"
    public index = 10000
    public blockNumbers = 10
    public endHeight = 460000

    private retryTime = initialRetryTime

    constructor(url: string, config: SingleNodeConfig = {}) {
        this.client = new MatrixRPC(url)
        this.logFileName = createLogFile()
    }

    private async catchWrapper(method: Function, ...args: any[]): Promise<SingleNodeResponse> {
        try {
            const result = args && args.length ? await method.apply(this.client, args) : await method.apply(this.client)

            const message = `[MATRIX REQUEST SUCCEEDED] ${method.name} - ${this.client.net}\n`
            console.log(message)
            this.retryTime = initialRetryTime

            // Log success message to log files. But it is unnecessary
            // const out = fs.createWriteStream(this.logFileName, { flags: 'a+' })
            // Readable.from(message).pipe(out)

            return singleNodeResponse('✔', result)
        } catch (e) {
            // console.log("[ERROR TRACE] -", e)  // Uncomment it to tracing for error

            if (e.message.includes('timeout')) {
                if (this.retryTime) {
                    this.retryTime = this.retryTime - 1
                    return this.catchWrapper(method, ...args)
                }
            }

            const message = `[MATRIX REQUEST FAILED] ${method.name} - ${e.message}\n`
            console.log(message)

            const out = fs.createWriteStream(this.logFileName, { flags: 'a+' })
            Readable.from(message).pipe(out)

            return singleNodeResponse('✘', message)
        }
    }

    public async listPlugins() {
        return await this.catchWrapper(this.client.listPlugins)
    }

    public async getApplicationLog() {
        return await this.catchWrapper(this.client.getApplicationLog, this.txid)
    }

    public async ping() {
        return await this.client.ping()
    }

    public async getBestBlockHash() {
        return await this.catchWrapper(this.client.getBestBlockHash)
    }

    public async getAccountState() {
        return await this.catchWrapper(this.client.getAccountState, this.address)
    }

    public async claimGas() {
        return await this.catchWrapper(this.client.claimGas, this.address)
    }

    public async dumpPrivateKey() {
        return await this.catchWrapper(this.client.dumpPrivateKey, this.address)
    }

    public async getAssetState() {
        return await this.catchWrapper(this.client.getAssetState, this.assetID)
    }

    public async getBalance() {
        const assetID = "025d82f7b00a9ff1cfe709abe3c4741a105d067178e645bc3ebad9bc79af47d4"
        return await this.catchWrapper(this.client.getBalance, assetID)
    }

    public async getBlock() {
        return await this.catchWrapper(this.client.getBlock, this.hash)
    }

    public async getBlockCount() {
        return await this.catchWrapper(this.client.getBlockCount)
    }

    public async getBlockHash() {
        return await this.catchWrapper(this.client.getBlockHash, this.index)
    }

    public async getBlockHeader() {
        return await this.catchWrapper(this.client.getBlockHeader, this.hash)
    }

    public async getBlockSysFee() {
        return await this.catchWrapper(this.client.getBlockSysFee, this.index)
    }

    public async getClaimable() {
        return await this.catchWrapper(this.client.getClaimable, this.address)
    }

    public async getConnectionCount() {
        return await this.catchWrapper(this.client.getConnectionCount)
    }

    public async getContractState() {
        const scriptHash = "dc675afc61a7c0f7b3d2682bf6e1d8ed865a0e5f"
        return await this.catchWrapper(this.client.getContractState, scriptHash)
    }

    public async getMetricBlockTimestamp() {
        return await this.catchWrapper(this.client.getMetricBlockTimestamp, this.blockNumbers, this.endHeight)
    }

    public async getNEP5Balances() {
        return await this.catchWrapper(this.client.getNEP5Balances, this.address)
    }

    public async getNEP5Transfers() {
        return await this.catchWrapper(this.client.getNEP5Transfers, this.address)
    }

    public async getNewAddress() {
        return await this.catchWrapper(this.client.getNewAddress)
    }

    public async getRawMemPool() {
        return await this.catchWrapper(this.client.getRawMemPool)
    }

    public async getRawTransaction() {
        return await this.catchWrapper(this.client.getRawTransaction, this.txid)
    }

    public async getStorage() {
        const scriptHash = "03febccf81ac85e3d795bc5cbd4e84e907812aa3"
        const key = "5065746572"
        return await this.catchWrapper(this.client.getStorage, scriptHash, key)
    }

    public async getTransactionHeight() {
        return await this.catchWrapper(this.client.getTransactionHeight, this.txid)
    }

    public async getTxOut() {
        const txid = "f4250dab094c38d8265acc15c366dc508d2e14bf5699e12d9df26577ed74d657"
        const n = 0
        return await this.catchWrapper(this.client.getTxOut, txid, n)
    }

    public async getPeers() {
        return await this.catchWrapper(this.client.getPeers)
    }

    public async getUnspents() {
        return await this.catchWrapper(this.client.getUnspents, this.address)
    }

    public async getVersion() {
        return await this.catchWrapper(this.client.getVersion)
    }

    public async getValidators() {
        return await this.catchWrapper(this.client.getValidators)
    }

    public async getWalletHeight() {
        return await this.catchWrapper(this.client.getWalletHeight)
    }

    public async importPrivateKey() {
        return await this.catchWrapper(this.client.importPrivateKey)
    }

    public async invokeFunction() {
        const scriptHash = "af7c7328eee5a275a3bcaee2bf0cf662b5e739be"
        const operation = "balanceOf"
        const params = [
            {
                "type": "Hash160",
                "value": "91b83e96f2a7c4fdf0c1688441ec61986c7cae26"
            }
        ]
        return await this.catchWrapper(this.client.invokeFunction, scriptHash, operation, params)
    }

    public async invokeScript() {
        const script = "00046e616d656724058e5e1b6008847cd662728549088a9ee82191"
        return await this.catchWrapper(this.client.invokeScript, script)
    }

    public async listAddress() {
        return await this.catchWrapper(this.client.listAddress)
    }

    public async sendRawTransaction() {
        const hex = ""
        return await this.catchWrapper(this.client.sendRawTransaction, hex)
    }

    public async sendFrom() {
        return await this.catchWrapper(this.client.sendFrom, this.assetID, this.address, this.address, 0)
    }

    public async sendToAddress() {
        return await this.catchWrapper(this.client.sendToAddress, this.assetID, this.address, 0)
    }

    // TODO: check is this argument are correct or is it need more information
    public async sendMany() {
        const outputsArray = [
            {
                "asset": "025d82f7b00a9ff1cfe709abe3c4741a105d067178e645bc3ebad9bc79af47d4",
                "value": 1,
                "address": "AbRTHXb9zqdqn5sVh4EYpQHGZ536FgwCx2"
            },
            {
                "asset": "025d82f7b00a9ff1cfe709abe3c4741a105d067178e645bc3ebad9bc79af47d4",
                "value": 1,
                "address": "AbRTHXb9zqdqn5sVh4EYpQHGZ536FgwCx2"
            }
        ]
        const fee = 0
        const address = "AbRTHXb9zqdqn5sVh4EYpQHGZ536FgwCx2"
        return await this.catchWrapper(this.client.sendMany, outputsArray, fee, address)
    }

    public async submitBlock() {
        const hex = ""
        return await this.catchWrapper(this.client.submitBlock, hex)
    }

    public async validateAddress() {
        return await this.catchWrapper(this.client.validateAddress, this.address)
    }
}
